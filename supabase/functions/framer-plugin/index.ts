  ...
  <body>
    ...
    <script type="module">
      ...
      window.runConfigure = async function () {
        if (!framer) { setStatus("Framer SDK not available. Open this inside Framer.", "error"); return; }
        setLoading(true);
...
serve(async (req: Request) => {
  ...
  // Serve framer.json manifest when Framer probes {url}/framer.json
  if (path.endsWith("/framer.json")) {
  ...
  // Serve the plugin HTML for all other requests (root, index.html, etc.)
  return new Response(PLUGIN_HTML, {
    headers: {
      ...corsHeaders,
