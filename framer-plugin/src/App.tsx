...
import { FIELDS, performSync } from "./sync"

interface AppProps { collection: ManagedCollection }
...
export default function App({ collection }: AppProps) {
    ...
    useEffect(() => {
        // Set up fields on first load
        collection.setFields([...FIELDS]).then(() => setConfigured(true))

        // Fetch article count + categories for the UI
    ...
    async function handleSync() {
        ...
        try {
            const count = await performSync(collection, selectedCategory)
            ...
            framer.notify("Sync failed", { variant: "error" })
        }
    }
    ...
    return (
        <main style={s.root}>
            ...
            <button style={{ ...s.btn, ...(syncing || !configured ? s.btnOff : {}) }} onClick={handleSync} disabled={syncing || !configured}>
            ...
            <div style={s.divider}/>
            <p style={s.hint}>Sync pulls all published articles from ContentLab into this collection.</p>
        </main>
    )
}
